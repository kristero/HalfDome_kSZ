...
def main():
    ...
    p.add_argument('--seeds',nargs='+',type=int,default=[20260920,20260921]);args=p.parse_args()
    ...
    quality=dict(count=len(ids),heldout=len(heldout),training_pool=len(train),
        ...
        scope='Experimental spherical point painter; numerical systematics are not certified')
    ...
    for size in sorted(set(n for n in [64,128,len(train)] if n<=len(train))):
        ...
        for method in ['bins40','pca','moped_unbinned']:
            clean_input=clean_unbinned if method=='moped_unbinned' else clean
            noisy_input=noisy_unbinned if method=='moped_unbinned' else noisy
            state,meta=transform_fit(clean_input[opt],noisy_input[opt],theta[opt],low,high,method)
            if method=='moped_unbinned':
                state['ell']=ell
                ...
                for seed in args.seeds:
                    ...
                    if size==len(train) and not shuffled:
                        ...
                        for label,observation in external_observations.items():
                            key='noisy_dl_unbinned' if method=='moped_unbinned' else 'noisy_dl'
                            ...
                            external[label]=dict(accepted=len(samples),proposals=draws,
                                ...
                                interpretation='Effective out-of-family posterior, not a FLAMINGO parameter truth')
