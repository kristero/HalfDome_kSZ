...
frb_host_mass_min = get_float_arg("frb_host_mass_min", apply_mass_cut ? mass_min : 0.0; env="FRB_HOST_MASS_MIN")
frb_host_mass_max = get_float_arg("frb_host_mass_max", Inf; env="FRB_HOST_MASS_MAX")
...
dm_los_mode = lowercase(get_string_arg("dm_los_mode", "shell"; env="DM_LOS_MODE"))
dm_los_apply_mass_cut = get_bool_arg("dm_los_apply_mass_cut", false; env="DM_LOS_APPLY_MASS_CUT")
dm_los_mass_max = get_float_arg("dm_los_mass_max", Inf; env="DM_LOS_MASS_MAX")
dm_los_shell_dz = get_float_arg("dm_los_shell_dz", 0.05; env="DM_LOS_SHELL_DZ")
save_healpix_maps = get_bool_arg("save_healpix_maps", save_healpix_maps; env="SAVE_HEALPIX_MAPS")
save_cl = get_bool_arg("save_cl", save_cl; env="SAVE_CL")
save_redshift_histograms = get_bool_arg("save_redshift_histograms", true; env="SAVE_REDSHIFT_HISTOGRAMS")
save_selected_halo_histogram = get_bool_arg("save_selected_halo_histogram", true; env="SAVE_SELECTED_HALO_HISTOGRAM")
...
save_only_cl = get_bool_arg("save_only_cl", false; env="SAVE_ONLY_CL")
...
if save_only_cl
    save_cl = true
    save_healpix_maps = false
    save_redshift_histograms = false
    save_frb_catalog = false
    save_dm_residual_diagnostic = false
end
...
dm_los_mode in ("shell", "direct") || error("Unsupported dm_los_mode=$(repr(dm_los_mode)). Use \"shell\" or \"direct\".")
...
frb_host_mass_min >= 0.0 || error("frb_host_mass_min must be nonnegative.")
isfinite(frb_host_mass_max) || (frb_host_mass_max == Inf) || error("frb_host_mass_max must be finite or Inf.")
frb_host_mass_max > frb_host_mass_min || error("frb_host_mass_max must be greater than frb_host_mass_min.")
...
dmlos_mass_max_val = Float64(dm_los_mass_max)
isfinite(dmlos_mass_max_val) || (dmlos_mass_max_val == Inf) || error("dm_los_mass_max must be finite or Inf.")
dmlos_mass_max_val > 0.0 || error("dm_los_mass_max must be positive.")
if dm_los_mode == "shell"
    dm_los_shell_dz > 0.0 || error("dm_los_shell_dz must be positive when dm_los_mode=\"shell\".")
end
...
function build_frb_host_mass_tag(mass_min_value::Real, mass_max_value::Real)
    upper_tag = isfinite(Float64(mass_max_value)) ? fmt_param_value(mass_max_value) : "inf"
    return "frbmass_$(fmt_param_value(mass_min_value))to$(upper_tag)"
end
...
function build_path_config_tag(;
    ...
    dm_los_mode::AbstractString,
    dm_los_apply_mass_cut::Bool,
    dm_los_mass_max::Real,
    dm_los_shell_dz::Real
)
    signature = join(
        (
            ...
            "dm_los_mode=$(dm_los_mode)",
            "dm_los_apply_mass_cut=$(dm_los_apply_mass_cut)",
            "dm_los_mass_max=$(Float64(dm_los_mass_max))",
            "dm_los_shell_dz=$(Float64(dm_los_shell_dz))"
        ),
        "|"
    )
...
path_config_tag = build_path_config_tag(
    ...
    dm_los_mode=dm_los_mode,
    dm_los_apply_mass_cut=dm_los_apply_mass_cut,
    dm_los_mass_max=dm_los_mass_max,
    dm_los_shell_dz=dm_los_shell_dz
)
...
frb_host_mass_tag = build_frb_host_mass_tag(frb_host_mass_min, frb_host_mass_max)
frb_weight_tag =
    frb_selection_mode == "redshift" ?
    "redshiftcorr_dz$(fmt_param_value(frb_redshift_weight_bin_width))" :
    "random_uniformhalo"
frb_tag = "nfrb$(frb_count)_seed$(frb_seed)_$(frb_host_mass_tag)_$(frb_weight_tag)_$(frb_redshift_tag)_$(run_config_tag)"
tsz_map_tag = "$(catalog_input_tag)_$(param_tag)_$(tsz_beam_tag)_$(tsz_path_config_tag)_$(mean_tag)"
frb_map_tag = "$(frb_tag)_$(mean_tag)"
dm_estimator_tag =
    dm_los_mode == "shell" ?
    "dmforeground_shells_dz$(fmt_param_value(dm_los_shell_dz))" :
    "dmforeground_directsorted"
dm_map_tag = "$(frb_tag)_$(param_tag)_$(dm_beam_tag)_$(dm_estimator_tag)_$(mean_tag)"
...
cl_tag_base = "$(frb_tag)_$(param_tag)_$(tsz_beam_tag)_$(dm_beam_tag)_$(dm_estimator_tag)_$(dm_residual_tag)_$(dm_cross_tag)"
...
frb_catalog_output_path = joinpath(
    output_dir,
    make_output_filename("$(catalog_tag)_FRB_catalog_nside$(nside)_$(frb_tag)_$(param_tag)_$(dm_estimator_tag)_$(dm_residual_tag)", ".h5")
)
...
dm_residual_diagnostic_output_path = joinpath(
    plot_output_dir,
    make_output_filename("FRB_DM_residual_diagnostic_$(catalog_tag)_$(frb_tag)_$(dm_estimator_tag)_$(dm_residual_tag)", ".png")
)
...
println("  frb_host_mass_min=$(frb_host_mass_min), frb_host_mass_max=$(frb_host_mass_max), frb_host_mass_tag=$(frb_host_mass_tag)")
...
println("  save_only_cl=$(save_only_cl), save_cl=$(save_cl), save_healpix_maps=$(save_healpix_maps)")
println("  save_redshift_histograms=$(save_redshift_histograms), save_frb_catalog=$(save_frb_catalog), save_dm_residual_diagnostic=$(save_dm_residual_diagnostic)")
println("  save_selected_halo_histogram=$(save_selected_halo_histogram)")
...
println("  dm_estimator_tag=$(dm_estimator_tag)")
println("  dm_cross_tag=$(dm_cross_tag)")
println("  dm_los_mode=$(dm_los_mode)")
println("  dm_los_apply_mass_cut=$(dm_los_apply_mass_cut), dm_los_mass_max=$(dm_los_mass_max)")
if dm_los_mode == "shell"
    println("  dm_los_shell_dz=$(dm_los_shell_dz)")
else
    println("  dm_los_shell_dz=$(dm_los_shell_dz)  # ignored in direct mode")
    println("  note: dm_los_mode=direct uses exact halo-by-halo LOS accumulation with chunk-wise redshift sorting.")
end
...
function sample_healpix_map_at_point(m::HealpixMap{<:Real, RingOrder}, ra::Real, dec::Real)
    theta = pi / 2 - Float64(dec)
    phi = Float64(ra)
    pix = Healpix.ang2pixRing(m.resolution, theta, phi)
    return Float64(m.pixels[pix])
end

function build_redshift_shell_edges(z_max::Real, dz::Real)
    z_max_f = Float64(z_max)
    dz_f = Float64(dz)
    z_max_f >= 0.0 || error("z_max must be nonnegative.")
    dz_f > 0.0 || error("dz must be positive.")

    if z_max_f == 0.0
        return [0.0, dz_f]
    end

    nshell = max(1, ceil(Int, z_max_f / dz_f))
    shell_edges = collect(range(0.0, nshell * dz_f; length=nshell + 1))
    shell_edges[end] = z_max_f
    return shell_edges
end

function assign_redshift_shell_index(z::Real, shell_edges::AbstractVector{<:Real})
    z_f = Float64(z)
    if !isfinite(z_f) || z_f < Float64(shell_edges[1]) || z_f > Float64(shell_edges[end])
        return 0
    end

    if z_f <= Float64(shell_edges[1])
        return 1
    end

    shell_idx = searchsortedfirst(shell_edges, z_f) - 1
    if shell_idx < 1
        return 1
    elseif shell_idx >= length(shell_edges)
        return length(shell_edges) - 1
    end

    return shell_idx
end

function append_dm_foreground_halos_to_shells!(
    dm_shell_ra::Vector{Vector{Float64}},
    dm_shell_dec::Vector{Vector{Float64}},
    dm_shell_mass::Vector{Vector{Float64}},
    dm_shell_redshift::Vector{Vector{Float64}},
    x,
    y,
    z,
    halo_mass,
    redshift;
    shell_edges,
    apply_mass_cut::Bool=false,
    mass_max::Float64=Inf
)
    z_max = Float64(shell_edges[end])
    sel = isfinite.(redshift) .& isfinite.(halo_mass) .& (redshift .>= 0.0) .& (redshift .<= z_max)
    if apply_mass_cut
        sel .&= halo_mass .<= mass_max
    end
    ...
    ys = Float64.(y[sel])
    zs = Float64.(z[sel])
    masses = Float64.(halo_mass[sel])
    redshifts = Float64.(redshift[sel])
    ras, decs = xyz_to_ra_dec_threaded(xs, ys, zs)

    added_count = 0
    @inbounds for i in eachindex(masses)
        shell_idx = assign_redshift_shell_index(redshifts[i], shell_edges)
        shell_idx == 0 && continue
        push!(dm_shell_ra[shell_idx], ras[i])
        push!(dm_shell_dec[shell_idx], decs[i])
        push!(dm_shell_mass[shell_idx], masses[i])
        push!(dm_shell_redshift[shell_idx], redshifts[i])
        added_count += 1
    end
...
function paint_dm_halo_slice!(
    tmp_dm_hp::HealpixMap{Float64, RingOrder},
    ...
    support_extra_radius_rad::Float64=0.0
)
    length(masses) == length(redshifts) == length(ras) == length(decs) || error(
        "DM halo slice inputs must have the same length."
    )
    isempty(masses) && return nothing

    perm = sortperm(decs)
    masses_sorted = Float64.(masses[perm])
    redshifts_sorted = Float64.(redshifts[perm])
    ras_sorted = Float64.(ras[perm])
    decs_sorted = Float64.(decs[perm])
    ...
    fill!(tmp_dm_hp.pixels, 0.0)
    paint!(tmp_dm_hp, workspace, dm_model_interp, masses_sorted, redshifts_sorted, ras_sorted, decs_sorted)
...
function compute_frb_dm_from_redshift_shells!(
    dm_hp::HealpixMap{Float64, RingOrder},
    tmp_dm_hp::HealpixMap{Float64, RingOrder},
    workspace::XGPaint.HealpixRingProfileWorkspace{Float64},
    dm_model_interp,
    frb_ras,
    ...
    halo_shell_decs,
    halo_shell_masses,
    halo_shell_redshifts,
    shell_edges;
    support_mask=nothing,
    support_extra_radius_rad::Float64=0.0
)
    length(frb_ras) == length(frb_decs) == length(frb_redshifts) || error("FRB inputs must have the same length.")
    length(halo_shell_ras) == length(halo_shell_decs) == length(halo_shell_masses) == length(halo_shell_redshifts) == (length(shell_edges) - 1) || error(
        "Shell DM halo inputs must match the number of redshift shells."
    )

    fill!(dm_hp.pixels, 0.0)
    sample_dm_raw = zeros(Float64, length(frb_ras))
    nshell = length(shell_edges) - 1
    frb_indices_by_shell = [Int[] for _ in 1:nshell]

    @inbounds for frb_idx in eachindex(frb_redshifts)
        shell_idx = assign_redshift_shell_index(frb_redshifts[frb_idx], shell_edges)
        shell_idx == 0 && error("Encountered FRB redshift $(frb_redshifts[frb_idx]) outside the shell range.")
        push!(frb_indices_by_shell[shell_idx], frb_idx)
    end

    painted_halo_count = 0
    @inbounds for shell_idx in 1:nshell
        shell_masses = halo_shell_masses[shell_idx]
        shell_redshifts = halo_shell_redshifts[shell_idx]
        shell_ras = halo_shell_ras[shell_idx]
        shell_decs = halo_shell_decs[shell_idx]

        if !isempty(shell_masses)
            paint_dm_halo_slice!(
                tmp_dm_hp,
                workspace,
                dm_model_interp,
                ...
                support_extra_radius_rad=support_extra_radius_rad
            )
            painted_halo_count += length(shell_masses)
        else
            fill!(tmp_dm_hp.pixels, 0.0)
        end

        z_lo = Float64(shell_edges[shell_idx])
        z_hi = Float64(shell_edges[shell_idx + 1])
        shell_width = max(z_hi - z_lo, eps(Float64))

        for frb_idx in frb_indices_by_shell[shell_idx]
            dm_before = sample_healpix_map_at_point(dm_hp, frb_ras[frb_idx], frb_decs[frb_idx])
            dm_shell = sample_healpix_map_at_point(tmp_dm_hp, frb_ras[frb_idx], frb_decs[frb_idx])
            alpha = clamp((Float64(frb_redshifts[frb_idx]) - z_lo) / shell_width, 0.0, 1.0)
            sample_dm_raw[frb_idx] = dm_before + alpha * dm_shell
        end

        dm_hp.pixels .+= tmp_dm_hp.pixels
    end

    return sample_dm_raw, painted_halo_count
end
...
thread_count_local() = isdefined(Base.Threads, :maxthreadid) ? Base.Threads.maxthreadid() : Base.Threads.nthreads()
thread_id_local() = Base.Threads.threadid()

function compute_frb_dm_direct_sorted!(
    dm_hp::HealpixMap{Float64, RingOrder},
    tmp_dm_hp::HealpixMap{Float64, RingOrder},
    workspace::XGPaint.HealpixRingProfileWorkspace{Float64},
    dm_model_interp,
    ...
    support_mask=nothing,
    support_extra_radius_rad::Float64=0.0,
    build_dm_profile_map::Bool=true
)
    length(frb_ras) == length(frb_decs) == length(frb_redshifts) || error("FRB inputs must have the same length.")
    ...
    max_frb_redshift = maximum(Float64.(frb_redshifts))
    frb_perm = sortperm(frb_redshifts)
    frb_redshifts_sorted = Float64.(frb_redshifts[frb_perm])
    frb_ux, frb_uy, frb_uz = ra_dec_to_unit_vectors(frb_ras, frb_decs)
    frb_ux_sorted = frb_ux[frb_perm]
    frb_uy_sorted = frb_uy[frb_perm]
    frb_uz_sorted = frb_uz[frb_perm]

    candidate_halo_count_ref = Ref(0)
    intersection_count_ref = Ref(0)
    ...
    stream_catalog_batches(
        ...
        valid = isfinite.(redshift) .& isfinite.(halo_mass) .& (redshift .>= 0.0) .& (redshift .<= max_frb_redshift)
        if apply_mass_cut
            valid .&= halo_mass .<= mass_max
        end
        any(valid) || return
        ...
        ys = Float64.(y[valid])
        zs = Float64.(z[valid])
        masses = Float64.(halo_mass[valid])
        redshifts = Float64.(redshift[valid])
        candidate_halo_count_ref[] += length(masses)

        if build_dm_profile_map || !isnothing(support_mask)
            ras, decs = xyz_to_ra_dec_threaded(xs, ys, zs)
            paint_dm_halo_slice!(
                tmp_dm_hp,
                workspace,
        ...
        perm = sortperm(redshifts)
        masses_sorted = masses[perm]
        redshifts_sorted = redshifts[perm]
        ux_sorted = ux[perm]
        uy_sorted = uy[perm]
        uz_sorted = uz[perm]

        cos_theta_max_sorted = Vector{Float64}(undef, length(masses_sorted))
        @inbounds for j in eachindex(masses_sorted)
            theta_max = Float64(compute_theta_max_local(dm_model_interp, masses_sorted[j] * XGPaint.M_sun, redshifts_sorted[j]))
            cos_theta_max_sorted[j] = (isfinite(theta_max) && theta_max > 0.0) ? cos(theta_max) : 2.0
        end

        thread_hit_counts = zeros(Int, thread_count_local())

        @threads :static for k in eachindex(frb_perm)
            z_frb = frb_redshifts_sorted[k]
            eligible_hi = searchsortedlast(redshifts_sorted, z_frb)
            eligible_hi == 0 && continue
            ...
            for j in 1:eligible_hi
                cosang = fx * ux_sorted[j] + fy * uy_sorted[j] + fz * uz_sorted[j]
                if cosang >= cos_theta_max_sorted[j]
                    theta = acos(clamp(cosang, -1.0, 1.0))
                    dm_sum += Float64(dm_model_interp(max(theta, theta_min), masses_sorted[j], redshifts_sorted[j]))
                    local_hits += 1
                end
            end

            sample_dm_raw[frb_perm[k]] += dm_sum
            thread_hit_counts[thread_id_local()] += local_hits
        end

        intersection_count_ref[] += sum(thread_hit_counts)
    end

    return sample_dm_raw, candidate_halo_count_ref[], intersection_count_ref[]
end

function apply_exact_host_dm_correction!(
    sample_dm_raw::Vector{Float64},
    sample_host_mass,
    sample_host_redshift,
    shell_edges,
    dm_model_interp;
    apply_mass_cut::Bool=false,
    mass_max::Float64=Inf
)
    length(sample_dm_raw) == length(sample_host_mass) == length(sample_host_redshift) || error(
        "Host DM correction inputs must have the same length."
    )

    theta_host = compute_theta_min_local(dm_model_interp)

    @inbounds for i in eachindex(sample_dm_raw)
        z_frb = Float64(sample_host_redshift[i])
        shell_idx = assign_redshift_shell_index(z_frb, shell_edges)
        shell_idx == 0 && error("Encountered FRB redshift $(z_frb) outside the shell range during host correction.")

        z_lo = Float64(shell_edges[shell_idx])
        z_hi = Float64(shell_edges[shell_idx + 1])
        shell_width = max(z_hi - z_lo, eps(Float64))
        alpha = clamp((z_frb - z_lo) / shell_width, 0.0, 1.0)
        host_dm_full = Float64(dm_model_interp(theta_host, Float64(sample_host_mass[i]), z_frb))
        host_in_shell_map = !apply_mass_cut || Float64(sample_host_mass[i]) <= mass_max
        correction_factor = host_in_shell_map ? (1.0 - alpha) : 1.0
        sample_dm_raw[i] += correction_factor * host_dm_full
    end
...
function append_frb_candidate_halos!(
    ...
    @inbounds for local_halo_index in eachindex(redshift)
        halo_mass_value = Float64(halo_mass[local_halo_index])
        if halo_mass_value < frb_host_mass_min || halo_mass_value >= frb_host_mass_max
            continue
        end
...
validate_healpix_map(::Nothing, ::AbstractString) = nothing
...
function mark_healpix_profile_support!(
    ...
    @inbounds for i in eachindex(masses)
        theta_max = Float64(compute_theta_max_local(profile_model, masses[i] * XGPaint.M_sun, redshifts[i]))
...
fill!(dm_hp.pixels, 0.0)
tmp_dm_hp = HealpixMap{Float64, RingOrder}(nside)
fill!(tmp_dm_hp.pixels, 0.0)
...
if reuse_cached_frb_hosts
    ...
    total_halo_count = stream_catalog_batches(
        ...
        append_frb_candidate_halos!(
            ...
            halo_mass,
            redshift;
            frb_host_mass_min=frb_host_mass_min,
            frb_host_mass_max=frb_host_mass_max,
    ...
    if frb_selection_mode == "redshift"
        println(
            "FRB host candidates in mass range [$(frb_host_mass_min), $(frb_host_mass_max)) " *
            "with z <= $(frb_z_max): $(eligible_frb_count)"
        )
    else
        println("FRB host candidates in mass range [$(frb_host_mass_min), $(frb_host_mass_max)): $(eligible_frb_count)")
    end

    frb_count <= eligible_frb_count || error(
        frb_selection_mode == "redshift" ?
            "frb_count=$(frb_count) exceeds the number of FRB host candidates in mass range " *
            "[$(frb_host_mass_min), $(frb_host_mass_max)) with z <= $(frb_z_max), which is $(eligible_frb_count)." :
            "frb_count=$(frb_count) exceeds the number of FRB host candidates in mass range " *
            "[$(frb_host_mass_min), $(frb_host_mass_max)), which is $(eligible_frb_count)."
    )
...
# Compute FRB DM from all foreground halos up to each FRB redshift.
...
if save_selected_halo_histogram
    ...
    println("Skipping selected FRB host halo histogram because save_selected_halo_histogram=false.")
end
...
if save_redshift_histograms
    positive_sample_redshift = sample_redshift_unsorted[sample_redshift_unsorted .> 0.0]
    isempty(positive_sample_redshift) && error("Need at least one positive sampled FRB redshift to make the histograms.")

    plot_z_linear_limits = expanded_linear_limits(0.0, maximum(sample_redshift_unsorted))
    ...
    if frb_selection_mode == "redshift"
        ...
        linear_expected_counts = expected_histogram_counts(pdf_z_grid_linear, pdf_values_linear, linear_bins, length(sample_redshift_unsorted))
    else
        reference_redshifts_linear = isnothing(eligible_frb_redshifts) ? sample_redshift_unsorted : eligible_frb_redshifts
    ...
    if frb_selection_mode == "redshift"
        ...
        log_expected_counts = expected_histogram_counts(pdf_z_grid_log, pdf_values_log, log_bins, length(sample_redshift_unsorted))
    else
        reference_redshifts_log_source = isnothing(eligible_frb_redshifts) ? sample_redshift_unsorted : eligible_frb_redshifts
    ...
    println("Skipping FRB redshift histogram plots because save_redshift_histograms=false.")
end

max_sample_redshift = maximum(sample_redshift_unsorted)
build_dm_profile_map = save_healpix_maps || apply_dm_gaussian_beam
dm_shell_ra = nothing
dm_shell_dec = nothing
dm_shell_mass = nothing
dm_shell_redshift = nothing

if dm_los_mode == "shell"
    dm_shell_edges = build_redshift_shell_edges(max_sample_redshift, dm_los_shell_dz)
    dm_shell_count = length(dm_shell_edges) - 1
    dm_shell_ra = [Float64[] for _ in 1:dm_shell_count]
    dm_shell_dec = [Float64[] for _ in 1:dm_shell_count]
    dm_shell_mass = [Float64[] for _ in 1:dm_shell_count]
    dm_shell_redshift = [Float64[] for _ in 1:dm_shell_count]
    dm_foreground_halo_count_ref = Ref(0)
    ...
    stream_catalog_batches(
        ...
        dm_foreground_halo_count_ref[] += append_dm_foreground_halos_to_shells!(
            dm_shell_ra,
            dm_shell_dec,
            dm_shell_mass,
            dm_shell_redshift,
            ...
            halo_mass,
            redshift;
            shell_edges=dm_shell_edges,
            apply_mass_cut=dm_los_apply_mass_cut,
            mass_max=dm_los_mass_max
        )
    end

    dm_foreground_halo_count_ref[] == 0 && error(
        "No foreground DM halos were collected with 0 <= z <= $(max_sample_redshift)." *
        (dm_los_apply_mass_cut ? " Try increasing dm_los_mass_max." : "")
    )

    nonempty_dm_shell_count = count(!isempty, dm_shell_mass)

    println(
        "Collected $(dm_foreground_halo_count_ref[]) DM foreground halos with 0 <= z <= $(max_sample_redshift)" *
        (dm_los_apply_mass_cut ? " and mass <= $(dm_los_mass_max)." : ".") *
        " Using $(dm_shell_count) redshift shells ($(nonempty_dm_shell_count) nonempty) with dz=$(dm_los_shell_dz)."
    )

    sample_dm_raw, dm_halos_used_count = compute_frb_dm_from_redshift_shells!(
        dm_hp,
        tmp_dm_hp,
        w,
        dm_model_interp,
        ...
        dm_shell_redshift,
        dm_shell_edges;
        support_mask=dm_support_mask,
        support_extra_radius_rad=dm_beam_support_extra_radius_rad
    )
    println("Computed per-FRB DM using $(dm_halos_used_count) foreground halos with cumulative redshift shells.")
    apply_exact_host_dm_correction!(
        sample_dm_raw,
        sample_mass_unsorted,
        sample_redshift_unsorted,
        dm_shell_edges,
    ...
    println("Applied exact host-halo DM correction on top of the shell-based foreground estimate.")
else
    sample_dm_raw, dm_halos_used_count, dm_intersection_count = compute_frb_dm_direct_sorted!(
        dm_hp,
        tmp_dm_hp,
        w,
        dm_model_interp,
        ...
        support_mask=dm_support_mask,
        support_extra_radius_rad=dm_beam_support_extra_radius_rad,
        build_dm_profile_map=build_dm_profile_map
    )
    println(
        "Computed per-FRB DM using exact direct LOS accumulation with chunk-wise redshift sorting over " *
        "$(dm_halos_used_count) candidate foreground halos and $(dm_intersection_count) LOS halo intersections."
    )
    if !build_dm_profile_map
        println("Skipped building the full DM profile map in direct mode because neither DM map saving nor DM beam smoothing is enabled.")
    end
end
...
healpix_maps_saved = false
if save_healpix_maps && !subtract_map_means
    ...
    healpix_maps_saved = true
end

if healpix_maps_saved
    frb_map = nothing
    dm_hp = nothing
    if use_dm_residual_for_cross
        dm_raw_frb_hp = nothing
    else
        dm_residual_hp = nothing
    end
    GC.gc()
end
...
if save_cl
    ...
    m_hp_cl_monopole_removed = nothing
    dm_analysis_hp_cl_monopole_removed = nothing
    GC.gc()
end
