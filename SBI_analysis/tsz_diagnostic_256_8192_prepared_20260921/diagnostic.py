...
def _run_row(row,theta,threads):
    ...
    settings.update(output_dir=str(out/'raw'),cache_dir=str(out/'cache'),nside='8192',
    ...
    env=dict(os.environ,FLAMINGO_CAMPAIGN=str(CAMPAIGN),HALFDOME_SOURCE_DIR=str(CAMPAIGN/'code/halfdome'),
        PREFLIGHT_OUTPUT=str(out),PREFLIGHT_GRID_FACTOR='1',VALIDATION_LEAN_MAPS='1',DIAGNOSTIC_SPLIT_SEEDS=','.join(map(str,seeds)),
...
def _summarize(count):
    ...
    atomic_json(out/'summary.json',dict(requested=count,completed=len(ids),rows=rows,
        geometry='sphere4; experimental point painter at raw8192/output4096',
...
def prepare(count):
    ...
    manifest=dict(count=count,parameter_order=NAMES,lower=LOW.tolist(),upper=HIGH.tolist(),
        ...
        raw_nside=8192,output_nside=4096,beam_fwhm_arcmin=2.,fsky=.4,mask_seed=12345,
