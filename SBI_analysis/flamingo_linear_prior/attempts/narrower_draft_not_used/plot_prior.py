...
def load_inputs(args):
    ...
    prior = module.UniformPrior(config)
    ...
    if not prior.contains(theta).all():
        raise ValueError("Some production rows fail the frozen uniform prior")
    ...
    for name, label, color, marker in zip(VARIANTS, FIT_LABELS, COLORS, MARKERS):
        ...
        if not good:
            raise ValueError("New FLAMINGO fit lies outside the new uniform prior: " + name)
        outside = (values < old_low) | (values > old_high)
        support[name] = {"inside_new_uniform_prior": good, "joint_metrics": metrics,
...
def legend_handles():
    return [Patch(facecolor=SAMPLE_COLOR, alpha=.55, label="Uniform prior design"),
...
def make_figures(data, args):
    ...
    fig.suptitle("Uniform prior and FLAMINGO fits", y=.995, fontsize=23)
    ...
    fig.text(.60, .91, "Uniform prior and FLAMINGO fits", fontsize=29, ha="center")
...
def main():
    ...
    for source in (args.production_root / "code", args.production_root / "design",
                   args.old_metadata.parent, args.fit_summary.parent):
    ...
    report = {"created_utc": datetime.now(timezone.utc).isoformat(),
              ...
              "marginal_histograms": "probability mass in equal-width physical bins for all nine parameters",
