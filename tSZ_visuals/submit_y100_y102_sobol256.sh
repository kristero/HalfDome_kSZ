...
submit_job() {
  ...
  local vars
  vars="$(qsub_var_list "${lightcone_id}" "${split}" "${model_exists}")"

  local cmd=("${QSUB}" -N "${job_name}" -v "${vars}")
...
y100_split1="$(submit_job 100 1 "${Y100_MODEL_EXISTS}")"
y100_split2="$(submit_job 100 2 "${Y100_MODEL_EXISTS}")"

if [[ "${DEPEND_Y102_ON_Y100}" == "true" ]]; then
  y102_split1="$(submit_job 102 1 "${Y102_MODEL_EXISTS}" "${y100_split1}")"
  y102_split2="$(submit_job 102 2 "${Y102_MODEL_EXISTS}" "${y100_split2}")"
else
  y102_split1="$(submit_job 102 1 "${Y102_MODEL_EXISTS}")"
  y102_split2="$(submit_job 102 2 "${Y102_MODEL_EXISTS}")"
fi
