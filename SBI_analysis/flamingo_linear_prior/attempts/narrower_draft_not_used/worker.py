...
from prior import UniformPrior, KEYS, digest, write_json
...
def run_row(root, run, theta, label):
    ...
    seed_row_id = index + (PREFLIGHT_OFFSET if is_preflight else 0)
    ...
    with (root/"locks"/lock_name).open("a") as lock:
        ...
        for index, values in rows:
            ...
            if target.exists():
                ...
                continue
            assert prior.contains(values), "Row outside frozen uniform box"
