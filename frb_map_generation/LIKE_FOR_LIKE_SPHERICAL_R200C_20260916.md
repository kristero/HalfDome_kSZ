...
**Lee22 through XGPaint's own pipeline (added 2026-09-17).** `Lee2022XGPaintDMProfile` (in `lee2022_frb_dm_profile.jl`) is an XGPaint `AbstractFRBProfile` that only supplies `XGPaint.get_params`: it returns the Lee22 parameters in XGPaint's gNFW convention, `beta = alpha beta' - gamma` (XGPaint's exponent is `-(beta+gamma)/alpha`, Lee22's is `-beta'`), and an amplitude `P0 = N n0 200 m_per_e / (0.9 X_H m_p)` that makes XGPaint's `rho_2d`/`ne2d`/`compute_DM` chain (with its `f_b rho_crit`, electron count and 0.9) return exactly `N n0 f(x) n200`. The mass and redshift dependence, including the M_cut broken power laws and the concentration term, is evaluated by the Lee22 code, so the mapping is exact for both fits, which a `PowerLawParam` representation could not be. Selected with `--dm-profile=lee2022_xgpaint`. Self-test: XGPaint's `compute_DM` on the wrapper matches the Lee22 code's projected column to 2e-6 and the 3-D density to 1e-9 at five (M, z, x) points for both fits. Products (`..._sphere1p0_m200c_lee22_{noconc,pref}_xh2_xgp_seed42/`, X_H^2 reading) are identical to the Lee22-code products bin for bin in every mass window (0 of 300 bins differ; means agree to 2e-16); figures `..._xgpaint_{upper_mass_limits,to_1e14}` overlay them as markers on the Lee22-code curves against TNG. In spherical mode the two share the chord quadrature and differ only in how the 3-D density is parameterized; the projected XGPaint chain itself is what the self-test exercises.
...
Because the normalization is a pure multiplicative factor on n_e, every Lee22 DM in the table scales exactly with it (only the zero fraction and the binning do not): the X_H/m_p alternative (0.578, i.e. 0.864 of the electron-count rows) would give 133-135 pc cm^-3 per hit ray in the 1e13-1e14 window, 0.96-0.98 of TNG, without a new run.

**X_H^2 reading, run (2026-09-17).** Option `--lee2022-normalization=hydrogen_count` sets n200 = 200 rho_cr f_b X_H/m_p, i.e. eq. 9 written in the form of eq. 7 with unit electron abundance; relative to the printed 1/(X_H m_p) this is the factor (X_H/m_p)/(1/(X_H m_p)) = X_H^2 = 0.578. Measured inside the sphere in the 1e13-1e14 window: mean DM per hit ray 133.1 (no-c) and 135.2 (best fit) against TNG's 138.6 (0.96 and 0.98), median 80.1 against 80.8, tail fractions 0.42 / 4.3 / 20.4 % below 3 / 10 / 30 pc cm^-3 against TNG's 0.57 / 4.5 / 20.1 %. Of the four readings this is the one that reproduces TNG's within-R200 DM in the fitted mass range; the electron-count reading is 11-13% high, the literal reading 66% high, the Omega_b/Omega_m reading a factor 3.8 low. Since the paper's n200 is printed with X_H in the denominator, this agreement is evidence that the fit used X_H in the numerator, but only the authors can confirm it. Figures `..._xh2_{upper_mass_limits,to_1e14}`; products `..._sphere1p0_m200c_lee22_{noconc,pref}_xh2_seed42/`.

**XGPaint-native reading, run (2026-09-17).** Option `--lee2022-normalization=xgpaint_ne2d` with `--dm-profile=lee2022_xgpaint` applies no hydrogen factor on the Lee22 side at all: the dimensionless profile n0 f(x) is taken as the gas density in units of 200 f_b rho_cr (P0 = 200 n0, exactly how XGPaint takes Battaglia16's rho_fit) and XGPaint's own `ne2d` converts gas mass to electrons with its own constants (the 0.9 and the mass per free electron of ionized H+He at X_H = 0.76). Relative to the printed eq. 9 this is 0.9 X_H m_p / m_per_e = 0.602. It is the reading in which Battaglia16 and Lee22 receive identical treatment in the pipeline. Measured inside the sphere in the 1e13-1e14 window: mean DM per hit ray 138.6 (no-c) and 140.8 (best fit) against TNG's 138.6 (1.00 and 1.02), median 83.5 against 80.8, tail fractions 0.39 / 4.0 / 19.5 % against 0.57 / 4.5 / 20.1 %. Figures `..._xgpnative_{upper_mass_limits,to_1e14}`; products `..._sphere1p0_m200c_lee22_{noconc,pref}_xgpnative_seed42/`. The z = 0 enclosed-baryon caveat of Section 5.3 applies to this reading as well (0.602/0.578 times the X_H^2 rows).
...
- The corrected Lee22 fits fall a factor 3.7 below TNG in the very mass range they were fitted to (1e13-10^14.8 h^-1 Msun). Geometry can no longer explain this. The sensitivity runs with the literal eq. 9 normalization (no extra Omega_b/Omega_m factor; everything else, including the sphere, the M_cut pivot, the TNG-mean concentration and the shape clip, identical) give 230-234 in the same window, a factor 1.66 *above* TNG, and their low-tail fractions (0.02 / 0.14 / 1.6 / 10 % below 1 / 3 / 10 / 30 pc cm^-3) are closer to TNG's than the corrected fits' (0.64 / 4.9 / 26 / 61 %), though not as close as Battaglia16's. Neither reading of the normalization reproduces TNG: the truth lies between them, at about 0.6 of the literal or 3.8 times the f_b-corrected value. The (1+z)^3/E^2 redshift-scaling hypothesis of the implementation check (factor 1.9 at z = 0.5) would close about half of the gap on top of the corrected reading. The Omega_b/Omega_m factor of the implementation check was inferred from the paper's Figure 5; the like-for-like numbers say that inference over-corrects, so the factor must be settled from the paper's definition of n200 and of the fitted quantity rather than from the figure. What was fitted is now known from the paper (Section 2.3): n_e per gas cell from TNG's electron abundance, V_i n_e,i = x_i m_i X_H/m_p (eq. 7); volume-weighted means in 27 radial bins over 0.04-1.34 R200 around Rockstar halo centres (eq. 8), i.e. all gas cells including satellites; normalization n200 = 200 rho_cr Omega_b/(X_H m_p Omega_m) exactly as printed (eq. 9). So the fitted profiles are means of the actual electron density, the quantity a within-R200 DM sum measures, and median stacking is excluded. If eq. 9 was used as printed, the literal reading is right by construction and the factor 0.60 has no explanation; if the code mirrored eq. 7 with unit electron abundance (X_H/m_p), the factor is X_H^2 = 0.578; if it used the ionized-gas electron count (1+X_H)/(2 m_p), the factor is 0.669 (next paragraph). The remaining things to confirm with the authors are the n200 actually used in the fit and, on Ralf's side, the cells included in the within-R200 sum.
...
| Lee22 no-c, n200 = 200 rho_cr f_b X_H/m_p (factor X_H^2), inside sphere | 64.9 | 120.3 | 185.4 | 94.8 | 0.242 | 0.04 | 0.44 | 4.18 | 18.58 |
| Lee22 best, n200 = 200 rho_cr f_b X_H/m_p (factor X_H^2), inside sphere | 64.9 | 128.1 | 197.4 | 98.9 | 0.242 | 0.03 | 0.44 | 4.13 | 18.20 |
| Lee22 no-c, XGPaint-native (P0 = 200 n0, ne2d electrons), inside sphere | 64.9 | 125.3 | 193.1 | 98.9 | 0.253 | 0.04 | 0.41 | 3.90 | 17.79 |
| Lee22 best, XGPaint-native (P0 = 200 n0, ne2d electrons), inside sphere | 64.9 | 133.4 | 205.6 | 103.1 | 0.253 | 0.03 | 0.41 | 3.86 | 17.42 |
...
| Lee22 no-c, n200 = 200 rho_cr f_b X_H/m_p (factor X_H^2), inside sphere | 53.7 | 71.5 | 133.1 | 80.1 | 0.242 | 0.05 | 0.42 | 4.31 | 20.43 |
| Lee22 best, n200 = 200 rho_cr f_b X_H/m_p (factor X_H^2), inside sphere | 53.7 | 72.6 | 135.2 | 80.1 | 0.242 | 0.05 | 0.41 | 4.23 | 20.06 |
| Lee22 no-c, XGPaint-native (P0 = 200 n0, ne2d electrons), inside sphere | 53.7 | 74.5 | 138.6 | 83.5 | 0.253 | 0.05 | 0.39 | 4.01 | 19.53 |
| Lee22 best, XGPaint-native (P0 = 200 n0, ne2d electrons), inside sphere | 53.7 | 75.7 | 140.8 | 83.5 | 0.253 | 0.05 | 0.39 | 3.92 | 19.14 |
...
| Lee22 no-c, n200 = 200 rho_cr f_b X_H/m_p (factor X_H^2), inside sphere | 60.2 | 76.6 | 127.3 | 76.8 | 0.242 | 0.05 | 0.53 | 4.98 | 22.07 |
| Lee22 best, n200 = 200 rho_cr f_b X_H/m_p (factor X_H^2), inside sphere | 60.2 | 77.7 | 129.2 | 76.8 | 0.242 | 0.05 | 0.53 | 4.94 | 21.78 |
| Lee22 no-c, XGPaint-native (P0 = 200 n0, ne2d electrons), inside sphere | 60.2 | 79.8 | 132.6 | 80.1 | 0.253 | 0.05 | 0.49 | 4.65 | 21.15 |
| Lee22 best, XGPaint-native (P0 = 200 n0, ne2d electrons), inside sphere | 60.2 | 81.0 | 134.6 | 80.1 | 0.253 | 0.05 | 0.50 | 4.62 | 20.85 |
