...
function run_maps()
    ...
    # A concrete element type is essential: Vector{Any} introduces dynamic
    # dispatch inside every halo/pixel update and destroys the measured speedup.
    states = [blank_state(8192) for _ in profiles]
    @assert isconcretetype(eltype(states)) && isconcretetype(eltype(profiles))
    ...
    for j in eachindex(cases)
        ...
        # Release the large map while retaining the concrete state element type.
        states[j] = blank_state(1); GC.gc()
    end
