# Copy of the FLAMINGO campaign adapter (process_maps.jl) that also accepts a HEALPix FITS y map
# as the signal source; the beam, mask, noise cache and spectrum code are the same, pixel for pixel.
using TOML, SHA, Pkg
...
function load_cache(campaign, cfg)
    ...
    for (key, actual) in runtime_info()
        ...
        key in ("threads", "adapter_sha256") && continue   # this adapter differs only in reading FITS input
...
function process_signal(campaign, output, input, cfg)
    ...
    if input == "halfdome"
        ...
        if endswith(lowercase(input), ".fits")
            # Unbeamed HEALPix Compton-y map painted by the repository's tSZ painter (RING, NSIDE 4096).
            println("Reading HEALPix FITS y map: ", input)
            signal = Healpix.readMapFromFITS(input, 1, Float64)
            @assert signal isa HealpixMap{Float64, RingOrder}
            @assert signal.resolution.nside == 4096
            @assert all(isfinite, signal.pixels)
        else
