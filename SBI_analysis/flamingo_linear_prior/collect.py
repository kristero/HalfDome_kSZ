...
def main():
    ...
    for name in ("noise_seed_row_ids","source_old_row","validation_split"):
        np.save(output/(name+".npy"),np.load(root/"design"/(name+".npy")))
    ...
    write_json(output/"complete.json",dict(rows=n,manifest_sha256=digest(root/"manifest.json"),
        ...
        prior="Linear-uniform base in all nine physical coordinates conditioned on the unchanged joint support; use code/sbi_prior.py and audit.json, not an unconditioned BoxUniform",
