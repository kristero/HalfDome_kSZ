...
**Electron-count reading (2026-09-17).** A third option, `--lee2022-normalization=electron_count`, replaces the 1/(X_H m_p) of eq. 9 by the free electrons per unit mass of ionized H+He, (1+X_H)/(2 m_p), a factor X_H(1+X_H)/2 = 0.669 on the literal reading. In the like-for-like spherical test it gives 154-157 pc cm^-3 per hit ray in the 1e13-1e14 window against TNG's 138.6 (1.11-1.13), with tail fractions close to TNG's. It is now the recommended reading, pending confirmation of how n200 was defined in the fit; items 1, 3 and 4 of this note are unchanged. Re-reading the paper (Section 2.3): eq. 7 gives V_i n_e,i = x_i m_i X_H/m_p from TNG's electron abundance x_i, eq. 8 takes volume-weighted means in 27 radial bins over 0.04-1.34 R200 of all gas cells around Rockstar halo centres, and eq. 9 prints n200 = 200 rho_cr Omega_b/(X_H m_p Omega_m), so the transcription in this note is correct and the profiles are means, not medians. The unresolved question is only whether the fit used eq. 9 as printed (literal, 1.66 times TNG), the electron count (0.669, 1.11 times TNG) or a X_H/m_p form mirroring eq. 7 (0.578). The last one was run as `--lee2022-normalization=hydrogen_count` and reproduces TNG within R200 in the fitted mass range: mean DM per hit ray 133-135 pc cm^-3 against 138.6, median 80.1 against 80.8, tail fractions within 10%. It is the reading that matches; the paper prints X_H in the denominator, so confirmation from the authors is still needed before adopting it as the default.
